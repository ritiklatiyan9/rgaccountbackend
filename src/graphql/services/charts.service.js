 ...
 */
export async function getRevenueVsExpense(siteId, start, end, resolution = 'MONTH') {
  ...
  const { rows } = await pool.query(
    `WITH first_entry AS (
       ...
       SELECT MIN(d) AS min_date FROM (
         SELECT MIN(date)    AS d FROM plot_payments            WHERE site_id = $1 AND date >= $2 AND date < $3
         UNION ALL
         SELECT MIN(pip.payment_date) FROM plot_installment_payments pip JOIN plots p ON p.id = pip.plot_id WHERE p.site_id = $1 AND pip.payment_date >= $2 AND pip.payment_date < $3
         UNION ALL
 ...
 */
export async function getProfitTrend(siteId, start, end, resolution = 'MONTH') {
  const data = await getRevenueVsExpense(siteId, start, end, resolution);
  return data.map(d => ({
    date: d.date,
    label: d.label,
