...
export async function up() {
  ...
  try {
    ...
    await db.query(
      "SELECT pg_advisory_xact_lock(hashtext('146_universal_transaction_transfers'))",
    );
    ...
    for (const side of ['source', 'target']) {
      await db.query(
        `ALTER TABLE transaction_entry_transfers DROP CONSTRAINT IF EXISTS transaction_entry_transfers_${side}_type_check`,
      );
      await db.query(
        `ALTER TABLE transaction_entry_transfers ADD CONSTRAINT transaction_entry_transfers_${side}_type_check CHECK (${side}_type IN (${types}))`,
      );
    }
